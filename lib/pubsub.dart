...
///
/// An absolute name of a topic starts with `/` and has the form:
///
///     /topics/<project-id>/<relative-name>
///
/// When a relative topic name is used, its absolute name is generated by
/// prepending `/topics/<project-id>/`, where `<project-id>` is the project
/// id passed to the constructor.
///
/// An absolute name of a subscription starts with `/` and has the form:
///
///     /subscriptions/<project-id>/<relative-name>
///
/// When a relative subscription name is used, its absolute name is
/// generated by prepending `/subscriptions/<project-id>/`, where
...
abstract class PubSub {
  ...
  /// Access Pub/Sub using an authenicated client.
  ///
  /// The [client] is an authentiacted HTTP client. This client must
  ...
  /// The [name] can be either an absolute name or a relative name.
  ///
  /// Returns a `Future` which completes with the newly created subscripton.
...
abstract class Topic {
  ...
  ///
  /// The message will get the labels specified in [labels]. The keys in this
  /// map must be strings and the values must be either Strings or integers.
  ///
  /// The [labels] are passed together with the message to the receiver.
  ///
  /// Returns a `Future` which completes with `null` when the operation
  /// is finished.
...
abstract class Subscription {
  ...
  ///
  /// A subscription without a configured endpoint URI is a pull subscripton.
...
/// set of labels (key-value pairs) associated with it.
///
...
abstract class Message {
  ...
  ///
  /// Message labels can be passed in the [labels] Map. The values in this
  /// map must be either Strings or integers. Integers must be positive
  /// 64-bit integers.
  factory Message.withString(String message, {Map<String, dynamic> labels}) =
  ...
  /// map must be either Strings or integers. Integers must be positive
  /// 64-bit integers.
  factory Message.withBytes(List<int> message, {Map<String, dynamic> labels}) =
